...
namespace DI.Services.Reports
{
    public class ReportProvider : ServiceBase, IReportProvider
    {
        ...
        public ReportProvider(ILoggerFactory logFactory, IOptionsMonitor<ReportConfig> reportOptionsMonitor) :
            base(logFactory)
        {
            reportOptionsMonitor.OnChange(config => { _reportConfig = config; });
