...
namespace Boards.Services.Access.Actions
{
    ...
    public class GrantAccessForm : DialogBuilder
    {
        ...
        {
            field.AddInput("Comments", "Comments", x => { x.FieldType = FormFieldType.Note; });
        }
    }

    public class GrantAccessDialog : BoardsDialogHandler<GrantAcccessModel>
    {
        public GrantAccessDialog(ILoggerFactory logFactory, IBoardsContext boardsContext) : base(logFactory,
            boardsContext)
        {
        }
        ...
        protected override async Task OnExecute(GrantAcccessModel model, IDictionary<string, object> data,
            long entityId)
        {
            ...
            if (teammUser == null)
                teammUser = await GetRepo<TeamUser>()
                    .CreateAsync(new TeamUser {AppTeamId = team.Id, AppUserId = user.Id});
            await SaveAsync();
            ...
            if (userRole == null)
                userRole = await GetRepo<UserRole>()
                    .CreateAsync(new UserRole {AppRoleId = role.Id, AppUserId = user.Id});
            await SaveAsync();
        }
    }
}