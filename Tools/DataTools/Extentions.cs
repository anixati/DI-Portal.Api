...
namespace DataTools
{
    public static class Extentions
    {
        ...
        public static string Limit(this string value, int length)
        {
            ...
            if (!string.IsNullOrEmpty(rx) && rx.Length > length) return rx[..length];
        ...
        public static string GetRefId(this string input)
        {
            ...
            var rm = input.Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
            return rm[1];
        }
        ...
        {
            if (!string.IsNullOrEmpty(value) && int.TryParse(value, out var ix)) return ix;
            return null;
        }

        public static decimal? ToDeci(this string value)
        {
            if (!string.IsNullOrEmpty(value) && decimal.TryParse(value, out var ix)) return ix;
            return null;
        }

        public static DateTime? ToDate(this string value)
        {
            if (!string.IsNullOrEmpty(value) && DateTime.TryParse(value, out var ix)) return ix;
            return null;
        }
